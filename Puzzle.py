...
class Board:
    """Board stores a given board configuration by a 2 dimensional array of Tiles, containing methods to extract data from the board. \n
        ...
        Properties:
            board: list of list of tiles, representing the list of rows of tiles on the board.
    ...
    def fill_tile(self, value: int, pos: Vector2):
        ...
        self.board[pos.y][pos.x] = Tile(value, pos, self.size)
...
class Puzzle:
    """A puzzle creates a Board using a given config, containing methods to find a solution.\n
        ...
        Properties:
            board: Board, initialised subject to the passed configuration.
    ...
    def check_if_valid(self) -> bool:
        """Identifies if the current board configuration is valid by checking each row, column and square has unique values amongst themselves."""
        for row in self.board.get_rows():
    ...
    def solve(self) -> Puzzle:
        ...
        while (not self.check_if_filled()) and self.is_solvable:
            self.prune_board() # Reduce state space of all tiles.
